...
const requiredHelpAssets = [
  ...
  "assets/help/telegram-help-card-rus-1.png",
  "assets/help/telegram-help-card-rus-2.png",
];
...
async function main() {
  ...
  const envExample = await fs.readFile(path.join(outRoot, "examples", "teledex.env.example"), "utf8");
  if (!/TELEDEX_BACKEND=app-server-v2/u.test(envExample) || !/TELEDEX_ENABLE_APP_SERVER_V2=1/u.test(envExample)) {
    fail("public env example must require Codez App Server v2");
  }
