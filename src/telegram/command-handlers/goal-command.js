...
function formatGoal(goal, language = DEFAULT_UI_LANGUAGE) {
  if (!goal) {
    return isEnglish(language)
      ? "No active goal is set for this app-server-v2 thread."
      : "Для этого app-server-v2 thread активная цель не задана.";
  }
  ...
  const prefix = isEnglish(language) ? "Goal" : "Цель";
...
function formatClearResult(result, language = DEFAULT_UI_LANGUAGE) {
  const cleared = result?.cleared === true;
  if (isEnglish(language)) {
    return cleared ? "Goal cleared." : "No goal was set.";
  }
  return cleared ? "Цель очищена." : "Цель не была задана.";
}

function formatGoalError(error, language = DEFAULT_UI_LANGUAGE) {
  const message = normalizeOptionalText(error?.message) || "unknown error";
  return isEnglish(language)
    ? `Goal RPC failed: ${message}`
    : `Goal RPC не выполнился: ${message}`;
}
...
export async function handleGoalCommand({
  ...
  if (parsed.action === "invalid") {
    return {
      responseText: isEnglish(language)
        ? "Usage: /goal [objective|set <objective>|pause|resume|complete|clear|budget <tokens>]"
        : "Формат: /goal [цель|set <цель>|pause|resume|complete|clear|budget <tokens>]",
  ...
  if (!isAppServerV2Enabled(config)) {
    return {
      responseText: isEnglish(language)
        ? "/goal is available only when app-server-v2 is enabled."
        : "/goal доступен только когда app-server-v2 включён.",
  ...
  if (!isCodexProviderSession(session)) {
    return {
      responseText: isEnglish(language)
        ? "/goal is available only for Codex app-server-v2 topics."
        : "/goal доступен только для Codex topics на app-server-v2.",
  ...
  if (!isAppServerV2Session(session)) {
    return {
      responseText: isEnglish(language)
        ? "/goal is available only for app-server-v2 Codex topics."
        : "/goal доступен только для Codex topics на app-server-v2.",
  ...
  if (!normalizeOptionalText(session.codex_thread_id)) {
    return {
      responseText: isEnglish(language)
        ? "Start one app-server-v2 run before using this /goal action."
        : "Сначала запусти один app-server-v2 run перед этим действием /goal.",
  ...
  if (executionHost?.ok === false) {
    ...
    return {
      responseText: isEnglish(language)
        ? `Execution host unavailable: ${hostLabel}`
        : `Execution host недоступен: ${hostLabel}`,
