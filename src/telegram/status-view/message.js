...
export function buildStatusMessage(
  ...
  const deepSeekTurnDisplay = deepSeekActiveTurnId
    ? `${deepSeekActiveTurnId} (${english ? "running" : "running"})`
    : deepSeekLiveTurnId
      ? `${deepSeekLiveTurnId} (${deepSeekLiveTurnStatus || (english ? "unknown" : "неизвестно")}, live)`
      : deepSeekLastTurnId ?? (english ? "unknown" : "неизвестно");
  ...
  return [
    ...
    ...(liveStatusWarning
      ? [
          `${english ? "live status refresh" : "live status refresh"}: ${liveStatusWarning}`,
        ]
      : []),
    ...
    ...(isDeepSeekHttpRuntime
      ? [
        `thread: ${session.codex_thread_id ?? (english ? "unknown" : "неизвестно")}`,
        `turn: ${deepSeekTurnDisplay}`,
      ]
      : []),
    `${english ? "language" : "язык"}: ${getLanguageLabel(language)}`,
    `${english ? "model" : "модель"}: ${displayModel ?? (english ? "unknown" : "неизвестно")}`,
    ...(isDeepSeekHttpRuntime
      ? []
      : [`${english ? "reasoning" : "reasoning"}: ${
        isDeepSeekRuntime
    ...
    `${english ? "context window" : "context window"}: ${formatNumber(deepSeekContextWindow ?? openRouterContextWindow ?? contextWindow, language)}`,
    isDeepSeekRuntime || isOpenRouterRuntime
      ? `${english ? "auto-compact" : "auto-compact"}: ${english ? "not applicable" : "не применяется"}`
      : `${english ? "auto-compact" : "auto-compact"}: ${formatNumber(autoCompactTokenLimit, language)}`,
    "",
    ...(isDeepSeekHttpRuntime
      ? [
        `${english ? "limits" : "лимиты"}: ${
          english
            ? "DeepSeek API (not tracked by gateway)"
            : "DeepSeek API (gateway не отслеживает)"
        }`,
      ]
      : isOpenRouterRuntime
        ? [
          `${english ? "limits" : "лимиты"}: ${
            english
              ? "OpenRouter API (not tracked by gateway)"
              : "OpenRouter API (gateway не отслеживает)"
          }`,
        ]
