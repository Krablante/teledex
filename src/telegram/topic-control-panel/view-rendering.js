...
function buildPendingInputLabel(kind, language = DEFAULT_UI_LANGUAGE) {
  return buildSharedPendingInputLabel(kind, language, {
    goal_text: isEnglish(language)
      ? "goal text; send the next text message"
      : "текст goal; отправь следующее текстовое сообщение",
...
function buildTopicControlPanelText({
  ...
  if (screen === "bot_settings") {
    if (deepSeekTopic || openRouterTopic) {
      return [
        english ? "Runtime settings" : "Настройки runtime",
        "",
  ...
  if (screen === "agent_model") {
    ...
    return [
      ...
      `${english ? "effective" : "effective"}: ${effectiveModel}`,
      "",
  ...
  if (screen === "agent_reasoning") {
    ...
    return [
      title,
      "",
      `${english ? "configured" : "настроено"}: ${formatTopicReasoningValue(configuredValue, { deepSeekTopic, openRouterTopic }, language)}`,
      `${english ? "effective" : "effective"}: ${formatTopicReasoningValue(effectiveReasoning, { deepSeekTopic, openRouterTopic }, language)}`,
      `${english ? "model basis" : "модель-основа"}: ${modelBasis}`,
      "",
