...
export function buildHookEconomyStatusLines(summary, language = "eng") {
  if (!summary || !summary.completedRuns) {
    return [];
  }

  const russian = language === "rus";
  ...
  return [
    `${russian ? "hook economy" : "hook economy"}: ${summary.completedRuns} completed, ~${saved} saved tokens`,
    ...(outputOriginal || outputVisible
      ? [`${russian ? "hook output bytes" : "hook output bytes"}: ${outputVisible} / ${outputOriginal} visible`]
      : []),
    ...(decisions ? [`${russian ? "hook decisions" : "hook decisions"}: ${decisions}`] : []),
    ...(plugins ? [`${russian ? "hook plugins" : "hook plugins"}: ${plugins}`] : []),
  ];
}
