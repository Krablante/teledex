...
function isGoalCompletionSummary(text) {
  ...
  return /^Goal (?:complete|achieved)\b[\s\S]*\bTime used:/iu.test(normalized)
    ...
    || /^Goal (?:закрыт|заверш[её]н)(?:\s|[.!:]|$)[\s\S]*Время по goal:/iu.test(normalized)
    || /^Цель (?:закрыта|завершена)(?:\s|[.!:]|$)[\s\S]*Время:/iu.test(normalized);
}
...
function isGoalRunFooter(text) {
  const normalized = String(text || "").trim();
  return /^Goal run:\s*(?:completed|заверш[её]н)(?:\s|$)/iu.test(normalized)
    && /(?:^|\n)(?:Time|Время|Tokens|Токены):/iu.test(normalized);
}
...
function formatTokenUsageLine(prefix, usage, language) {
  if (!usage || typeof usage !== "object") {
    return getSessionUiLanguage({ ui_language: language }) === "eng"
      ? `${prefix.eng}: unavailable`
      : `${prefix.rus}: недоступно`;
  }
...
function formatGoalRunTokenUsage(usage, language) {
  const lines = [
    formatTokenUsageLine(
      { eng: "Tokens", rus: "Токены" },
...
function appendGoalRunFooter({
  ...
  const isEnglish = getSessionUiLanguage({ ui_language: language }) === "eng";
  const duration = selectGoalFooterDuration({ run, state, finishedAt });
  const usage = selectGoalFooterTokenUsage(state);
  const lines = [
    isEnglish ? "Goal run: completed" : "Goal run: завершён",
    duration
      ? (isEnglish ? `Time: ${duration}` : `Время: ${duration}`)
      : null,
