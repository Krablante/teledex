...
test("handleIncomingMessage reports unavailable local host for /new", async () => {
  ...
  assert.match(sent[0].text, /хосте local/u);
...
test("handleIncomingMessage reports runtime selection failures for /new in English from General", async () => {
  ...
  assert.doesNotMatch(sent[0].text, /Неверный/u);
});

test("buildNewTopicRuntimeSelectionErrorMessage localizes known Russian details", () => {
  ...
  const text = buildNewTopicRuntimeSelectionErrorMessage(error, "rus");

  assert.match(text, /Неверный runtime\/model/u);
  assert.match(text, /Неподдерживаемая OpenRouter model: bad model/u);
  assert.doesNotMatch(text, /Unsupported/u);
});
