...
test("handleIncomingMessage shows /q status with queued prompt previews", async () => {
  ...
  assert.match(sent[0].text, /Очередь Agent: 2/u);
...
test("handleIncomingMessage accepts /кю as a queue command alias", async () => {
  const sent = [];
  const session = {
    session_key: "-1000000:77",
    chat_id: "-1000000",
  ...
  const result = await handleIncomingMessage({
    ...
    sessionService: {
      ...
      async listPromptQueue() {
        return [
          { raw_prompt: "queued through Cyrillic alias" },
        ];
      },
      async recordHandledSession() {
        return session;
  ...
  assert.match(sent[0].text, /queued through Cyrillic alias/u);
});
...
test("preparePromptRoutingContext treats blank /кю as a queue-buffer flush only", async () => {
  let flushed = false;
  const result = await preparePromptRoutingContext({
    botUsername: "gatewaybot",
    message: {
