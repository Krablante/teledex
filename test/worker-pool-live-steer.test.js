...
test("CodexWorkerPool live-steers DeepSeek Codex-provider runs through same-thread exec recovery", async () => {
  ...
  await workerPool.startPromptRun({
    session,
    prompt: "Начни DeepSeek provider задачу.",
    message: {
      message_id: 730,
  ...
  const steered = await workerPool.steerActiveRun({
    session,
    rawPrompt: "Live steer: добавь эту деталь в текущий run.",
    message: {
      message_id: 731,
      message_thread_id: 20332,
    },
  ...
  assert.match(runCalls[1].prompt, /Live steer: добавь эту деталь/u);
  assert.equal(runtimeEvents[2]?.details.recovery_kind, "live-steer-restart");
  assert.equal(sentMessages.at(-1)?.text, "DeepSeek учёл live steer.");
...
test("CodexWorkerPool keeps recovering after each newly accepted live steer", async () => {
          ...
          await onEvent(
            {
              kind: "agent_message",
              eventType: "item.completed",
              text: "Учёл все live steer и продолжил run.",
              messagePhase: "final_answer",
            },
            {
              type: "item.completed",
  ...
  assert.equal(sentMessages.at(-1)?.text, "Учёл все live steer и продолжил run.");
