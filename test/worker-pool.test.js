...
test("CodexWorkerPool uses the last goal progress message when app-server-v2 finalizes with a completion summary", async () => {
  ...
  const workerPool = new CodexWorkerPool({
    ...
    runTask: ({ onEvent }) => ({
      ...
      finished: (async () => {
        ...
        await onEvent({
          kind: "agent_message",
          text: "Goal закрыт. Время по goal: 4 секунды.",
          messagePhase: "final_answer",
        });
        return {
          backend: "app-server-v2",
  ...
  assert.match(reloaded.last_agent_reply, /Токены: total=120/u);
  assert.match(reloaded.last_agent_reply, /Токены \(raw\): total=120, input=100, output=20, reasoning=5/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /Goal закрыт/u);
  assert.equal(
    sentMessages.some((payload) => (
	      typeof payload.text === "string"
	      && payload.text.includes(token)
	      && payload.text.includes("Токены: total=120")
	      && payload.text.includes("Токены (raw): total=120")
	    )),
    true,
  );
});
...
test("CodexWorkerPool completes app-server-v2 goal runs with an empty final answer after update_goal complete", async () => {
  ...
  assert.equal(reloaded.last_run_status, "completed");
  assert.match(reloaded.last_agent_reply, new RegExp(`^${token}\\n\\nGoal run: заверш`, "u"));
  assert.match(reloaded.last_agent_reply, /Токены: total=1512/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /Не смог закончить run/u);
  assert.equal(
    sentMessages.some((payload) => (
      typeof payload.text === "string"
      && payload.text.includes(token)
      && payload.text.includes("Токены: total=1512")
    )),
    true,
  );
});
...
test("CodexWorkerPool strips trailing manual goal summary before appending the canonical footer", async () => {
      ...
      finished: (async () => {
        ...
        await onEvent({
          ...
          text: [
            ...
            "Проверка: passed.",
            "",
            "Goal закрыт. Время по goal: 9s, токены: 400.",
          ].join("\n"),
          messagePhase: "final_answer",
        });
        return {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.equal(reloaded.last_run_status, "completed");
  assert.match(reloaded.last_agent_reply, /^Готово\.\n\nПроверка: passed\./u);
  assert.match(reloaded.last_agent_reply, /\n\nGoal run: завершён\n/u);
  assert.match(reloaded.last_agent_reply, /Токены: total=400/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /Goal закрыт/u);
  assert.equal(
    sentMessages.some((payload) => (
      typeof payload.text === "string"
      && payload.text.includes("Проверка: passed.")
      && payload.text.includes("Goal run: завершён")
      && !payload.text.includes("Goal закрыт")
    )),
    true,
  );
});
...
test("CodexWorkerPool replaces a manual canonical goal footer with computed metrics", async () => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /^Готово\.\n\nGoal run: завершён/u);
  assert.match(reloaded.last_agent_reply, /Токены: total=40/u);
  assert.match(reloaded.last_agent_reply, /Токены \(raw\): total=120, input=100, cached=80, output=20, reasoning=5/u);
...
test("CodexWorkerPool goal footer falls back to raw total for partial run usage", async () => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /Токены: total=400/u);
  assert.match(reloaded.last_agent_reply, /Токены \(raw\): total=400, output=25/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /Токены: total=25/u);
});
...
test("CodexWorkerPool goal footer reports cumulative app-server run token delta", async () => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /Токены: total=60/u);
  assert.match(reloaded.last_agent_reply, /Токены \(raw\): total=250, input=210, cached=190, output=40, reasoning=15/u);
...
test("CodexWorkerPool goal footer keeps run tokens across fresh-thread recovery", async () => {
  ...
  assert.match(
    ...
    /Токены \(raw\): total=220, input=150, cached=100, output=70, reasoning=12/u,
  );
  assert.doesNotMatch(reloaded.last_agent_reply, /Токены: total=70/u);
});
...
test("CodexWorkerPool clears active turn state across replacement-thread token domains", async () => {
  ...
  assert.match(reloaded.last_agent_reply, /Токены: total=70/u);
  assert.match(
    reloaded.last_agent_reply,
    /Токены \(raw\): total=90, input=50, cached=20, output=40, reasoning=7/u,
  );
  assert.doesNotMatch(reloaded.last_agent_reply, /Токены: total=110/u);
...
test("CodexWorkerPool keeps completion-only turn usage in run token accounting", async () => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /Токены: total=11/u);
  assert.match(
    reloaded.last_agent_reply,
    /Токены \(raw\): total=13, input=10, cached=2, output=3/u,
  );
  const finishedEvent = runtimeEvents.find((event) => event.type === "run.finished");
  assert.deepEqual(finishedEvent?.details.token_usage, {
    input_tokens: 10,
...
test("CodexWorkerPool goal footer uses blended Teledex run tokens and preserves raw metrics", async (t) => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /Время: 3263s/u);
  assert.match(reloaded.last_agent_reply, /Токены: total=834048/u);
  assert.match(
    reloaded.last_agent_reply,
    /Токены \(raw\): total=33101952, input=32990004, cached=32267904, output=111948, reasoning=22640/u,
  );
  assert.doesNotMatch(reloaded.last_agent_reply, /Время: 42s/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /Токены: total=33101952/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /37119/u);
  assert.deepEqual(reloaded.last_token_usage, {
    input_tokens: 1100,
    cached_input_tokens: 1000,
...
test("CodexWorkerPool goal footer falls back to native goal tokens without run usage", async () => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /Время: \d+s/u);
  assert.match(reloaded.last_agent_reply, /Токены: total=37119/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /Токены \(raw\)/u);
});
...
test("CodexWorkerPool goal footer does not reuse stale token usage", async () => {
  ...
  const reloaded = await sessionStore.load(resumedSession.chat_id, resumedSession.topic_id);
  assert.match(reloaded.last_agent_reply, /Токены: недоступно/u);
  assert.doesNotMatch(reloaded.last_agent_reply, /999/u);
  assert.equal(
    sentMessages.some((payload) => (
      typeof payload.text === "string"
      && payload.text.includes(token)
      && payload.text.includes("Токены: недоступно")
...
test("CodexWorkerPool uses goal ack as initial progress and holds neutral spinner", async () => {
  ...
  const finishRun = createDeferred();
  const ackText = [
    "Goal принят; запущено app-server-v2 продолжение.",
    "",
    "Цель: проверить порядок ack.",
    "Status: active",
  ].join("\n");
