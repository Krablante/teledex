...
test("handleIncomingMessage queues plain follow-up while /compact is rebuilding the brief", async () => {
  ...
  const result = await handleIncomingMessage({
    ...
    message: {
      message_id: 882,
      text: "не трогай тот долгий процесс",
      from: { id: 1001001001, is_bot: false },
      chat: { id: -1000000 },
      message_thread_id: 771,
    },
  ...
  assert.equal(queued[0].entry.rawPrompt, "не трогай тот долгий процесс");
  assert.equal(queued[0].entry.prompt, "User Prompt:\nне трогай тот долгий процесс");
...
test("handleIncomingMessage defers live steer while compact recovery has an active run", async () => {
  ...
  const result = await handleIncomingMessage({
    ...
    message: {
      message_id: 883,
      text: "учти вторую правку прямо в текущем run",
      from: { id: 1001001001, is_bot: false },
      chat: { id: -1000000 },
      message_thread_id: 771,
    },
  ...
  assert.equal(steered[0].rawPrompt, "учти вторую правку прямо в текущем run");
