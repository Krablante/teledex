...
test("handleIncomingMessage sends the help card from General topic", async () => {
  ...
  assert.equal(documents[0].document.fileName, "teledex-help-summer-rus-1.png");
  assert.equal(documents[1].document.fileName, "teledex-help-summer-rus-2.png");
...
test("handleIncomingMessage sends the English help card from ENG General", async () => {
  ...
  assert.equal(result.command, "help");
  assert.equal(documents.length, 2);
  assert.equal(documents[0].document.fileName, "teledex-help-summer-eng-1.png");
  assert.equal(documents[1].document.fileName, "teledex-help-summer-eng-2.png");
});
...
test("handleIncomingMessage falls back to complete help text when help-card delivery fails", async () => {
  ...
  assert.match(sent[0].text, /\/q <text> \| \/кю <text>/u);
});
...
test("handleIncomingMessage sends the guidebook PDF from General topic", async () => {
  ...
  assert.equal(documents[0].document.fileName, "teledex-guidebook-rus.pdf");
  assert.match(documents[0].document.filePath, /teledex-guidebook-rus\.pdf$/u);
