...
test("extractBotCommand treats /кю as the /q queue command alias", () => {
  const command = extractBotCommand(
    {
      text: "/кю@gatewaybot status",
    },
    "gatewaybot",
  );

  assert.equal(command.name, "q");
  assert.equal(command.raw, "/кю@gatewaybot");
