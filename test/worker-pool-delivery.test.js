...
test("CodexWorkerPool never leaks noisy shell wrapper commands into progress", async () => {
  ...
  const noisyTelegramCommand =
    `/bin/bash -lc "sleep 29 && node --input-type=module -e 'const payload = { chat_id: -1000000, message_thread_id: 1560, text: \\"Тест\\" }; const res = await fetch(\\"https://api.telegram.org/botTOKEN/sendMessage\\", { method: \\"POST\\" });'"`;
  const workerPool = new CodexWorkerPool({
    api: {
      async sendMessage(payload) {
        sentMessages.push(payload);
